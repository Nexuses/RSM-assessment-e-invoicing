...
| *(next app)* | `…` | `…` | `3001` | `…_assessments` | `…` | Reuse same Postgres; **new DB only** |
| *(next app)* | `…` | `…` | `3002` | `…` | `…` | |
| *(next app)* | `…` | `…` | `3003` | `…` | `…` | |
...
For a NEW assessment app on the same VPS:
  1) Pick unique PORT (3001+), PM2 name, nginx domain, DB name
...
Full detail: SHARED-INFRA.md in RSM-assessment-e-invoicing-1
```

---

## 10. Related docs in the reference repo

| File | Purpose |
|------|---------|
| [`DEPLOY.md`](DEPLOY.md) | Full VPS deploy for the e-invoicing app |
| [`docker-compose.yml`](docker-compose.yml) | Shared Postgres definition (source of truth) |
| [`.env.example`](.env.example) | Env var template |
| [`ecosystem.config.cjs`](ecosystem.config.cjs) | PM2 example (`PORT` 3000) |
