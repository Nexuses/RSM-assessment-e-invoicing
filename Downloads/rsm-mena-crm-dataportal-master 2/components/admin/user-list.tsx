...
import { AddAlternativeEmailDialog } from "./add-alternative-email-dialog"
...
export const UserList = forwardRef<UserListRef, UserListProps>(({ onAddUser }, ref) => {
  ...
  const [isAlternativeEmailDialogOpen, setIsAlternativeEmailDialogOpen] = useState(false)
  ...
  const handleAddAlternativeEmail = (userId: string, email: string) => {
    setSelectedUserId(userId)
    setSelectedUserEmail(email)
    setIsAlternativeEmailDialogOpen(true)
  }
        ...
        return (
          <DropdownMenu>
            ...
            <DropdownMenuContent align="end" className="bg-zinc-900 border-zinc-800">
              ...
              <DropdownMenuItem 
                onClick={() => handleAddAlternativeEmail(user.id, user.email)}
                className="text-purple-400 hover:bg-zinc-800 hover:text-purple-400 focus:bg-zinc-800 focus:text-purple-400"
              >
  ...
  return (
    <>
      ...
      />
      <AddAlternativeEmailDialog
        userId={selectedUserId || ""}
        userEmail={selectedUserEmail}
        isOpen={isAlternativeEmailDialogOpen}
        onClose={() => {
          setIsAlternativeEmailDialogOpen(false)
          setSelectedUserId(null)
          setSelectedUserEmail("")
        }}
        onSuccess={() => {
          // No need to refresh user list - alternative emails aren't displayed in the table
          // The dialog handles its own state updates
        }}
      />
